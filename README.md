...
比較了三個方式，其實並沒有看出哪種classifiers能夠提供較好的預測，而所嘗試的方法也無法有效改善預測的結果。
這很大概就是garbage in, grabage out的概念，使用了不太有關聯的數據，就無法得到有意義結果。
不過也可能是因為我對於資料分析的資歷尚淺，所以無法有效利用手邊的數據與工具所導致。

### 突發奇想的嘗試
重新檢視之前的預測結果後，在SVM的圖上看到可能的線索。下圖中紫色標記處可能就每年的差異。
![](https://imgur.com/YnGTab8.png)

而為了要減少年間的差異，因此利用了下面的程式碼，讓每一年的數據自己做一次標準化(EX：2009年的數據用2009年的數據標準化)。
![](https://imgur.com/6WFmcGG.png)

之後再使用下面的程式碼，找出較佳的準確度以及對應的penalty。其中linear模型的預測結果與前面並無差異，因此僅附上sigmoid模型的程式碼與結果。
![](https://imgur.com/G9DH3Rd.png)

所得到的結果如下，在penalty = 0.04的情形下有最佳的準確度51.59%，比之前多了1%左右的預測準確度，為目前所有嘗試中最高的準確度。
![](https://imgur.com/m1MD4EX.png)

雖然準確度終於有所提升了，但是預測成功率也還是在擲銅板等級。
而由上面的圖片其實也可以看出用來區分的邊界其實並沒有相當理想，所以也不能算是一個成功的結果。
